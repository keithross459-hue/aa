...
def build_product_pdf(product: Dict[str, Any]) -> bytes:
...
def build_product_bundle_zip(
    ...
    """Zip containing product.pdf, product.md, campaigns.md, tiktok.md, sales_copy.txt."""
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        z.writestr("product.pdf", build_product_pdf(product))
...
def build_library_zip(items: List[Dict[str, Any]]) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        for it in items:
            ...
            z.writestr(f"{safe}/product.pdf", build_product_pdf(p))
